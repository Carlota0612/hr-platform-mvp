...
(async () => {
  try {
    ...
    result = await request('/api/users', { method:'POST', body: JSON.stringify({name:'No Token', email:'no-token@example.com', roles:['user']}) });
    assert(result.response.status === 403, 'CSRF protection did not reject missing token');

    result = await request('/api/users', { method:'POST', headers:{'X-CSRF-Token':token}, body: JSON.stringify({name:'Combo', email:'combo@example.com', roles:['admin','hr']}) });
    ...
    assert(result.body.invite_url, 'invite URL was not returned');
    const inviteToken = new URL(result.body.invite_url).searchParams.get('token');
    result = await request(`/api/invitations/${inviteToken}`);
    assert(result.response.ok && result.body.email === 'combo@example.com', 'invite lookup failed');
    result = await request('/api/login', { method:'POST', body: JSON.stringify({email:'combo@example.com', password:'StrongPass123!'}) });
    assert(!result.response.ok, 'invited user should not login before accepting invite');
    result = await request('/api/invitations/accept', { method:'POST', body: JSON.stringify({token: inviteToken, password:'NewStrongPass123!'}) });
    assert(result.response.ok, 'invite accept failed');
    result = await request('/api/login', { method:'POST', body: JSON.stringify({email:'combo@example.com', password:'NewStrongPass123!'}) });
    assert(result.response.ok, 'invited user could not login with their own password');
