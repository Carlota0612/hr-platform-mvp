...
(async () => {
  try {
    ...
    result = await request('/api/invitations/accept', {
      method:'POST',
      body: JSON.stringify({
        token: inviteToken,
    ...
    result = await request('/api/hr-info/2', {
      method:'PUT',
      headers:{'X-CSRF-Token':token},
      body: JSON.stringify({
        salary:'123456',
        ...
        job_grade:'G7',
        birthday:'1990-01-01',
        salary_effective_date:'2026-06-24'
      })
    });
    ...
    result = await request(`/api/tasks/${taskId}/status`, {
      method:'PUT',
      headers:{'X-CSRF-Token':token},
      body: JSON.stringify({
        status:'Done'
    ...
    result = await request('/api/settings', {
      ...
      headers:{'X-CSRF-Token':token},
      body: JSON.stringify({
        taskStatuses:['Not started','In progress','Done','On hold','Blocked']
      })
    });
