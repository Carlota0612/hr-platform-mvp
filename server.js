...
function decryptHrRow(row) {
  ...
  for (const field of ['salary', 'bonus', 'visa_status', 'visa_expiry_date', 'job_grade', 'birthday']) {
    row[field] = decryptSecret(row[field]);
  }
...
function primaryRole(roles) {
  return roles.includes('admin') ? 'admin' : roles.includes('hr') ? 'hr' : roles.includes('manager') ? 'manager' : 'user';
}
...
function hasRole(req, role) {
  return rolesForUser(req.session.user).includes(role);
}

function requireAdmin(req, res, next) {
  if (!req.session.user || !hasRole(req, 'admin')) return res.status(403).json({ error: 'Admin only' });
  next();
}

function requireHr(req, res, next) {
  if (!req.session.user || !(hasRole(req, 'hr') || hasRole(req, 'admin'))) return res.status(403).json({ error: 'HR only' });
  next();
}
...
function isHr(req) {
  return hasRole(req, 'hr');
}

function isUser(req) {
  return hasRole(req, 'user') || rolesForUser(req.session.user).length === 0;
}

async function auditLog(req, action, targetType, targetId = null, details = {}) {
  await run(
    'INSERT INTO audit_logs (user_id,user_email,action,target_type,target_id,details,ip,user_agent) VALUES (?,?,?,?,?,?,?,?)',
    [
...
async function initDb() {
  ...
  await run(`CREATE TABLE IF NOT EXISTS career_progression (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    employee_id INTEGER NOT NULL,
    from_role TEXT,
    to_role TEXT,
  ...
  await run(`CREATE TABLE IF NOT EXISTS employee_private_info (
  ...
  if (isProduction && adminPassword === 'CHANGE_ME_IN_RENDER') {
  ...
  if (cnt.n === 0) {
    ...
    await run(
      'INSERT INTO employees (first_name,last_name,title,email,location,team_id,level,skills,bio) VALUES (?,?,?,?,?,?,?,?,?)',
      ['Paolo', 'Rossi', 'Research Manager', 'paolo@example.com', 'Switzerland', 1, 'M4', 'Battery modelling, BMS, MATLAB', 'Example manager profile']
    );
    await run(
      'INSERT INTO employees (first_name,last_name,title,email,location,team_id,manager_id,level,skills,bio) VALUES (?,?,?,?,?,?,?,?,?,?)',
      ['Maya', 'Chen', 'Researcher', 'maya@example.com', 'Switzerland', 1, 1, 'IC3', 'Python, Energy storage, Optimization', 'Example employee profile']
    );
  }
}
...
app.get('/api/me', (req, res) => {
  res.json({ user: req.session.user ? { ...req.session.user, csrfToken: req.session.csrfToken } : null });
});

app.get('/api/dashboard', requireAuth, async (req, res) => {
  if (isAdmin(req)) {
    const [employees, teams, openActions, devPlans, projects, openTasks] = await Promise.all([
      get('SELECT COUNT(*) n FROM employees WHERE status="Active"'),
      get('SELECT COUNT(*) n FROM teams'),
      get('SELECT COUNT(*) n FROM action_plans WHERE status != "Done"'),
      get('SELECT COUNT(*) n FROM development_plans WHERE status != "Done"'),
      get('SELECT COUNT(*) n FROM projects'),
      get('SELECT COUNT(*) n FROM tasks WHERE status != "Done"')
    ]);

    return res.json({
      employees: employees.n,
      teams: teams.n,
      openActions: openActions.n,
      developmentPlans: devPlans.n,
      projects: projects.n,
...
async function scopedRows(req, table) {
  ...
  if (table === 'employees') {
    return allowed.length ? all(`SELECT * FROM employees WHERE id IN (${placeholders(allowed)}) ORDER BY id DESC`, allowed) : [];
  }
  ...
  if (['teams', 'projects'].includes(table)) {
    return isManager(req) ? all(`SELECT * FROM ${table} ORDER BY id DESC`) : [];
  }
...
async function canWrite(req, table, body, id = null) {
  ...
  if (isManager(req) && ['tasks', 'action_plans', 'development_plans', 'achievements'].includes(table)) {
    return managed.includes(target);
  }
...
function crud(table, fields) {
  app.get(`/api/${table}`, requireAuth, async (req, res) => {
    res.json(await scopedRows(req, table));
  });

  app.post(`/api/${table}`, requireAuth, async (req, res) => {
    if (!(await canWrite(req, table, req.body))) return res.status(403).json({ error: 'Not allowed' });

    const cols = fields.filter(f => req.body[f] !== undefined);
    const vals = cols.map(f => req.body[f]);
    const q = `INSERT INTO ${table} (${cols.join(',')}) VALUES (${cols.map(() => '?').join(',')})`;
    const r = await run(q, vals);

    res.json(await get(`SELECT * FROM ${table} WHERE id=?`, [r.lastID]));
  });

  app.put(`/api/${table}/:id`, requireAuth, async (req, res) => {
    if (!(await canWrite(req, table, req.body, req.params.id))) return res.status(403).json({ error: 'Not allowed' });
...
app.put('/api/settings', requireAdmin, async (req, res) => {
  for (const key of ['theme', 'tabs', 'dashboardCards', 'taskStatuses']) {
    if (req.body[key]) {
      await run('INSERT OR REPLACE INTO app_settings (key,value) VALUES (?,?)', [key, JSON.stringify(req.body[key])]);
    }
  }
...
app.put('/api/tasks/:id/status', requireAuth, async (req, res) => {
  if (!(await canWrite(req, 'tasks', { assigned_employee_id: req.body.assigned_employee_id, status: req.body.status }, req.params.id))) {
    return res.status(403).json({ error: 'Not allowed' });
  }

  await run('UPDATE tasks SET status=? WHERE id=?', [req.body.status, req.params.id]);
  await auditLog(req, 'update_task_status', 'task', req.params.id, { status: req.body.status });

  res.json(await get('SELECT * FROM tasks WHERE id=?', [req.params.id]));
});

app.get('/api/users', requireAdmin, async (req, res) => {
  res.json(await all('SELECT id,name,email,role,employee_id,force_password_change,created_at FROM users ORDER BY id DESC'));
});
...
app.get('/api/audit-logs', requireAdmin, async (req, res) => {
  res.json(await all('SELECT * FROM audit_logs ORDER BY id DESC LIMIT 250'));
});

app.post('/api/users', requireAdmin, async (req, res) => {
  const { name, email, employee_id, password = 'ChangeMe123!' } = req.body;
  ...
  const r = await run(
    'INSERT INTO users (name,email,role,employee_id,password_hash,force_password_change) VALUES (?,?,?,?,?,?)',
    [name, email, roles.join(','), employee_id || null, hashPassword(password), 1]
  );

  await auditLog(req, 'create_user', 'user', r.lastID, { roles });

  res.json(await get('SELECT id,name,email,role,employee_id,force_password_change,created_at FROM users WHERE id=?', [r.lastID]));
});
...
app.put('/api/users/:id', requireAdmin, async (req, res) => {
  ...
  if (body.roles || body.role) body.role = normalizeRoles(body.roles || body.role).join(',');

  if (body.role) {
    try {
      await assertNotLastAdmin(req.params.id, body.role);
    } catch (err) {
      return res.status(400).json({ error: err.message });
    }
  }

  const cols = ['name', 'email', 'role', 'employee_id', 'force_password_change'].filter(f => body[f] !== undefined);
  const vals = cols.map(f => f === 'employee_id' && body[f] === '' ? null : body[f]);
  ...
  if (password) {
    ...
    cols.push('force_password_change');
    vals.push(1);
  }

  await run(`UPDATE users SET ${cols.map(f => `${f}=?`).join(',')} WHERE id=?`, [...vals, req.params.id]);
  await auditLog(req, 'update_user', 'user', req.params.id, { roles: body.role });

  res.json(await get('SELECT id,name,email,role,employee_id,force_password_change,created_at FROM users WHERE id=?', [req.params.id]));
});
...
app.put('/api/hr-info/:employeeId', requireHr, async (req, res) => {
  ...
  await run(`INSERT INTO employee_private_info (employee_id,salary,bonus,visa_status,visa_expiry_date,job_grade,birthday,updated_at)
    ...
    [
      employeeId,
      encryptSecret(values.salary),
      encryptSecret(values.bonus),
      encryptSecret(values.visa_status),
      encryptSecret(values.visa_expiry_date),
      encryptSecret(values.job_grade),
      encryptSecret(values.birthday)
    ]
  );
  ...
  if (salaryChanged) {
    await run(
      'INSERT INTO salary_history (employee_id,salary,effective_date,notes) VALUES (?,?,?,?)',
      [
        employeeId,
        encryptSecret(values.salary),
        req.body.salary_effective_date || new Date().toISOString().slice(0, 10),
        encryptSecret(req.body.salary_notes || 'Salary updated')
      ]
    );
  }
...
app.get('/api/hr-info/:employeeId/salary-history', requireHr, async (req, res) => {
  await auditLog(req, 'view_salary_history', 'salary_history', req.params.employeeId);

  const rows = await all('SELECT * FROM salary_history WHERE employee_id=? ORDER BY effective_date DESC, id DESC', [req.params.employeeId]);

  res.json(rows.map(r => ({
    ...r,
    salary: decryptSecret(r.salary),
    notes: decryptSecret(r.notes)
  })));
});

app.post('/api/hr-info/:employeeId/salary-history', requireHr, async (req, res) => {
  const r = await run(
    'INSERT INTO salary_history (employee_id,salary,effective_date,notes) VALUES (?,?,?,?)',
    [
      req.params.employeeId,
      encryptSecret(req.body.salary),
      req.body.effective_date,
      encryptSecret(req.body.notes)
    ]
  );

  await auditLog(req, 'create_salary_history', 'salary_history', req.params.employeeId);

  const row = await get('SELECT * FROM salary_history WHERE id=?', [r.lastID]);

  res.json({
    ...row,
    salary: decryptSecret(row.salary),
    notes: decryptSecret(row.notes)
  });
});
...
app.post('/api/import/employees', requireAdmin, upload.single('file'), async (req, res) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (existing) {
        await run(
          `UPDATE employees SET first_name=?, last_name=?, title=?, location=?, manager_id=?, team_id=?, start_date=?, level=?, status=?, skills=?, bio=? WHERE id=?`,
          [
            values.first_name,
            values.last_name,
            values.title,
        ...
        await run(
          `INSERT INTO employees (first_name,last_name,title,email,location,manager_id,team_id,start_date,level,status,skills,bio) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
          [
            values.first_name,
            values.last_name,
            values.title,
...
app.get('/api/people-directory', requireAuth, async (req, res) => {
  ...
  res.json(await all(
    `SELECT e.*, t.name AS team_name, m.first_name || ' ' || m.last_name AS manager_name
     FROM employees e
     LEFT JOIN teams t ON e.team_id=t.id
     LEFT JOIN employees m ON e.manager_id=m.id
     WHERE e.id IN (${placeholders(ids)})
       AND (e.first_name LIKE ? OR e.last_name LIKE ? OR e.title LIKE ? OR e.skills LIKE ? OR t.name LIKE ?)
...
app.get('/api/org-chart', requireAuth, async (req, res) => {
  if (isAdmin(req) || isHr(req)) {
    return res.json(await all(
      `SELECT e.id, e.first_name || ' ' || e.last_name AS name, e.title, e.manager_id, t.name AS team
       FROM employees e
       LEFT JOIN teams t ON e.team_id=t.id
  ...
  res.json(await all(
    `SELECT e.id, e.first_name || ' ' || e.last_name AS name, e.title, e.manager_id, t.name AS team
     FROM employees e
     LEFT JOIN teams t ON e.team_id=t.id
     WHERE e.status='Active' AND e.id IN (${placeholders(ids)})
     ORDER BY e.manager_id`,
    ids
  ));
});
...
app.get('/api/employees/:id/profile', requireAuth, async (req, res) => {
  ...
  const [employee, achievements, progression, actions, development] = await Promise.all([
    get(
      'SELECT e.*, t.name AS team_name, m.first_name || " " || m.last_name AS manager_name FROM employees e LEFT JOIN teams t ON e.team_id=t.id LEFT JOIN employees m ON e.manager_id=m.id WHERE e.id=?',
      [id]
    ),
...
app.post('/api/ai/talent-memo/:id', requireAuth, async (req, res) => {
  const profile = await (await fetch(`http://localhost:${PORT}/api/employees/${req.params.id}/profile`, {
