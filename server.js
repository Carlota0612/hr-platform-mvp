...
app.post('/api/users', requireAdmin, async (req,res)=>{ const {name,email,employee_id,password='ChangeMe123!'} = req.body; const passwordError = validatePassword(password); if (passwordError) return res.status(400).json({ error: passwordError }); const roles = normalizeRoles(req.body.roles || req.body.role); const r = await run('INSERT INTO users (name,email,role,employee_id,password_hash,force_password_change) VALUES (?,?,?,?,?,?)', [name,email,roles.join(','),employee_id || null,hashPassword(password),1]); await auditLog(req, 'create_user', 'user', r.lastID, { roles }); res.json(await get('SELECT id,name,email,role,employee_id,force_password_change,created_at FROM users WHERE id=?', [r.lastID])); });
