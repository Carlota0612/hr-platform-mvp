...
if (!sessionSecret) {
  throw new Error('SESSION_SECRET must be configured in production. In Render, add a SESSION_SECRET environment variable. Generate one with: npm run generate-secrets');
}

const hrEncryptionSecret = process.env.HR_ENCRYPTION_KEY || (isProduction ? null : 'dev-only-hr-encryption-key');
if (!hrEncryptionSecret) {
  throw new Error('HR_ENCRYPTION_KEY must be configured in production. In Render, add an HR_ENCRYPTION_KEY environment variable. Generate one with: npm run generate-secrets. Keep this value stable; changing it will prevent previously encrypted HR data from decrypting.');
}
...
async function auditLog(req, action, targetType, targetId = null, details = {}) {
  await run('INSERT INTO audit_logs (user_id,user_email,action,target_type,target_id,details,ip,user_agent) VALUES (?,?,?,?,?,?,?,?)', [
...
app.post('/api/login', async (req, res) => {
  ...
  if (user.force_password_change) {
    return res.status(403).json({ error: 'Please use your invitation link to set your password before signing in.' });
  }
...
function crud(table, fields) {
  ...
  app.put(`/api/${table}/:id`, requireAuth, async (req, res) => {
    ...
    const cols = fields.filter(f => req.body[f] !== undefined);
    await run(`UPDATE ${table} SET ${cols.map(f => `${f}=?`).join(',')} WHERE id=?`, [...cols.map(f => req.body[f]), req.params.id]);
...
app.put('/api/settings', requireAdmin, async (req, res) => {
  for (const key of ['theme', 'tabs', 'dashboardCards', 'taskStatuses']) {
    if (req.body[key]) await run('INSERT OR REPLACE INTO app_settings (key,value) VALUES (?,?)', [key, JSON.stringify(req.body[key])]);
  }
  res.json({ ok: true });
});
...
app.post('/api/users', requireAdmin, async (req, res) => {
  ...
  const r = await run('INSERT INTO users (name,email,role,employee_id,password_hash,force_password_change) VALUES (?,?,?,?,?,?)', [
    name,
    email,
    roles.join(','),
    employee_id || null,
  ...
  const invite = await createUserInvite(req, r.lastID, email);
  const emailResult = await sendInviteEmail({ ...invite, email, name });

  await auditLog(req, 'create_user_invite', 'user', r.lastID, { roles, emailSent: emailResult.sent });

  const user = await get('SELECT id,name,email,role,employee_id,force_password_change,created_at FROM users WHERE id=?', [r.lastID]);
  res.json({ ...user, ...invite, email_sent: emailResult.sent, email_note: emailResult.reason || null });
});
...
app.put('/api/hr-info/:employeeId', requireHr, async (req, res) => {
  ...
  if (salaryChanged) {
    await run('INSERT INTO salary_history (employee_id,salary,effective_date,notes) VALUES (?,?,?,?)', [
      employeeId,
...
app.post('/api/hr-info/:employeeId/salary-history', requireHr, async (req, res) => {
  const r = await run('INSERT INTO salary_history (employee_id,salary,effective_date,notes) VALUES (?,?,?,?)', [
  ...
  res.json({ ...row, salary: decryptSecret(row.salary), notes: decryptSecret(row.notes) });
});
...
app.get('/api/org-chart', requireAuth, async (req, res) => {
  if (isAdmin(req) || isHr(req)) {
    return res.json(await all(`SELECT e.id, e.first_name || ' ' || e.last_name AS name, e.title, e.manager_id, t.name AS team FROM employees e LEFT JOIN teams t ON e.team_id=t.id WHERE e.status='Active' ORDER BY e.manager_id`));
  }
