...
function allowedPages(){ return navPages.filter(p => isAdmin() || isHr() || !['settings','users','audit','teams','projects','hr'].includes(p)); }
...
async function loadInvitation(){
  ...
  if(!token){ $('login').innerHTML = '<h1>Invalid invite</h1><p class="muted">This invitation link is missing its token.</p>'; return; }
  try {
  ...
  } catch(e) { $('login').innerHTML = `<h1>Invite unavailable</h1><p class="muted">${escapeHtml(e.message)}</p>`; }
}
async function acceptInvitation(token){
  if($('invitePassword').value !== $('invitePasswordConfirm').value) return alert('Passwords do not match.');
  try { await api('/api/invitations/accept',{method:'POST',body:JSON.stringify({token,password:$('invitePassword').value})}); alert('Your password is set. Please sign in.'); location.href = '/'; } catch(e) { alert(e.message); }
}
...
function inputFor(field, table, value = ''){
  ...
  if(field==='status') { const options = table === 'tasks' ? taskStatusOptions() : ['Active','Planning','Open','In progress','Blocked','Done','Archived']; return `<select name="status">${options.map(o=>`<option ${selectedAttr(o, value)}>${o}</option>`).join('')}</select>`; }
...
async function createRow(e, table){ e.preventDefault(); try { const result = await api(`/api/${table}`, {method:'POST', body:JSON.stringify(formDataWithoutBlanks(e.target))}); if(table==='users' && result.invite_url) prompt(result.email_sent ? 'Invite email sent. Copy link if needed:' : `Email not sent (${result.email_note || 'not configured'}). Copy this invite link:`, result.invite_url); e.target.reset(); editing = null; await loadTable(table); } catch(err) { alert(err.message); } }
...
async function resendInvite(id){ try{ const r = await api(`/api/users/${id}/invite`, {method:'POST', body:JSON.stringify({})}); prompt(r.email_sent ? 'Invite email sent. Copy link if needed:' : `Email not sent (${r.email_note || 'not configured'}). Copy this invite link:`, r.invite_url); await loadTable('users'); }catch(e){ alert(e.message); } }
